...
class _ChartAxis {
      ...
      if (axis.associatedAxisName != null) {
        for (int i = 0;
          ...
          if (axis.associatedAxisName ==
            ...
            _calculateCrossingValues(axis, axis._crossAxis);
          }
        }
      } else {
  ...
  void _calculateCrossingValues(ChartAxis currentAxis, ChartAxis targetAxis) {
    ...
    if (axis.isVisible) {
      ...
      if (axis._orientation == AxisOrientation.horizontal) {
        if (!axis.opposedPosition) {
          axis._totalSize +=
              _bottomAxes.isNotEmpty && axis.labelStyle.fontSize > 0
          ...
          axis._totalSize += _topAxes.isNotEmpty && axis.labelStyle.fontSize > 0
              ? _axisPadding.toDouble()
              : 0;
          if (axis._crossValue != null && axis._crossRange != null) {
            final num crosPosition =
        ...
        if (!axis.opposedPosition) {
          axis._totalSize +=
              _leftAxes.isNotEmpty && axis.labelStyle.fontSize > 0
                  ? _axisPadding.toDouble()
                  : 0;
          if (axis._crossValue != null && axis._crossRange != null) {
            final num crosPosition =
          ...
          axis._totalSize +=
              _rightAxes.isNotEmpty && axis.labelStyle.fontSize > 0
                  ? _axisPadding.toDouble()
                  : 0;
          if (axis._crossValue != null && axis._crossRange != null) {
            final num crosPosition =
  ...
  void _calculateLeftAxesBounds() {
    ...
    for (int axisIndex = 0; axisIndex < axesLength; axisIndex++) {
      ...
      axis._bounds = Rect.fromLTWH(axisSize, rect.top + axis.plotOffset, width,
          rect.height - 2 * axis.plotOffset);
    }
  }
  ...
  void _calculateVisibleAxes() {
    ...
    if (visibleSeries.isNotEmpty) {
      ...
      final List<String> axisNames = <String>[];
      for (int axisIndex = 0;
          axisIndex < _chartWidget.axes.length;
          axisIndex++) {
        axisNames.add(_chartWidget.axes[axisIndex]._name);
      }
      for (int axisIndex = 0;
        ...
        if (axis._orientation == null && _chartWidget.indicators.isNotEmpty) {
          for (int i = 0; i < _chartWidget.indicators.length; i++) {
            if (_chartWidget.indicators[i].isVisible) {
              if (_chartWidget.indicators[i].xAxisName == axis._name) {
                axis._orientation = _chartWidget._requireInvertedAxis
                    ? AxisOrientation.vertical
                    : AxisOrientation.horizontal;
              } else if (_chartWidget.indicators[i].yAxisName == axis._name) {
                axis._orientation = _chartWidget._requireInvertedAxis
                    ? AxisOrientation.horizontal
                    : AxisOrientation.vertical;
              }
