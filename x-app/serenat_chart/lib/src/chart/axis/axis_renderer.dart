...
class _ChartAxisRenderer with _CustomizeAxisElements {
  ...
  void drawHorizontalAxesLine(
    ...
    final _CustomPaintStyle paintStyle = _CustomPaintStyle(
        ...
        axis._axisRenderer.getAxisLineColor(axis) ??
            chart._chartState._chartTheme.axisLineColor,
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (needAnimate) {
          final double oldLocation =
              _getPrevLocation(axis, oldAxis, tempInterval);
          ...
          _drawDashedPath(
              ...
              _CustomPaintStyle(
                  ...
                  axis._axisRenderer.getAxisMajorTickColor(axis, i) ??
                      chart._chartState._chartTheme.majorTickLineColor,
                  PaintingStyle.stroke),
  ...
  void drawHorizontalAxesMajorGridLines(Canvas canvas, Offset point,
    ...
    final _CustomPaintStyle paintStyle = _CustomPaintStyle(
        ...
        axis._axisRenderer.getAxisMajorGridColor(axis, index) ??
            chart._chartState._chartTheme.majorGridLineColor,
        PaintingStyle.stroke);
    _drawDashedPath(
        canvas,
        paintStyle,
  ...
  void drawVerticalAxesMajorGridLines(Canvas canvas, Offset point,
    ...
    final _CustomPaintStyle paintStyle = _CustomPaintStyle(
        ...
        axis._axisRenderer.getAxisMajorGridColor(axis, index) ??
            chart._chartState._chartTheme.majorGridLineColor,
        PaintingStyle.stroke);
    if (chart.primaryXAxis._axisRenderer._xAxisStart !=
  ...
  void drawHorizontalAxesLabels(
    ...
    if (renderType == axis.labelPosition.toString().split('.')[1]) {
      ...
      final List<AxisLabel> visibleLabels = axis._visibleLabels;
      num tempInterval, pointX, pointY;
      for (int i = 0; i < visibleLabels.length; i++) {
        ...
        textStyle = _getTextStyle(
            ...
            fontColor: textStyle.color ??
                chart._chartState._chartTheme.axisLabelColor);
        ...
        pointX =
            _getShiftedPosition(axis, axisBounds, pointX, pointY, textSize, i);
        ...
        if (axis.labelIntersectAction == AxisLabelIntersectAction.hide &&
            ...
            (!axis.isInversed
                ? pointX < visibleLabels[i - 1]._labelRegion.right
                : visibleLabels[i]._labelRegion.right >
                    visibleLabels[i - 1]._labelRegion.left)) {
          axis._visibleLabels[i]._needRender = false;
          continue;
        }
        ...
        if (needAnimate) {
          final double oldLocation =
              _getPrevLocation(axis, oldAxis, tempInterval, textSize, angle);
          pointX = oldLocation != null
              ? (oldLocation - (oldLocation - pointX) * animationFactor)
              : pointX;
        }
    ...
    if (axis.labelPosition.toString().split('.')[1] == renderType) {
      ...
      for (dynamic i = 0; i < visibleLabels.length; i++) {
        ...
        if (axis.labelAlignment == LabelAlignment.center) {
          pointX = location.x;
          pointY = location.y;
        } else if (axis.labelAlignment == LabelAlignment.far) {
          pointX = location.x;
          pointY = location.y - textSize.height / 2;
        } else if (axis.labelAlignment == LabelAlignment.near) {
          pointX = location.x;
          pointY = location.y + textSize.height / 2;
        }
        if (axis.labelIntersectAction == AxisLabelIntersectAction.hide &&
        ...
        if (axis.edgeLabelPlacement == EdgeLabelPlacement.shift) {
          ...
          } else if (i == axis._visibleLabels.length - 1 &&
        ...
        if (needAnimate) {
          final double oldLocation =
              _getPrevLocation(axis, oldAxis, tempInterval, textSize);
          pointY = oldLocation != null
              ? (oldLocation - (oldLocation - pointY) * animationFactor)
              : pointY;
        }
  ...
  double _getPrevLocation(ChartAxis axis, ChartAxis oldAxis, dynamic value,
    ...
    if (oldAxis._visibleRange.minimum > value) {
      location = axis._orientation == AxisOrientation.vertical
          ? (axis.isInversed ? bounds.top : bounds.bottom)
          : (axis.isInversed ? bounds.right : bounds.left);
    } else if (oldAxis._visibleRange.maximum < value) {
      location = axis._orientation == AxisOrientation.vertical
          ? (axis.isInversed ? bounds.bottom : bounds.top)
          : (axis.isInversed ? bounds.left : bounds.right);
    } else {
      if (axis._orientation == AxisOrientation.vertical) {
        location =
            (_valueToCoefficient(value, oldAxis) * oldAxis._bounds.height) +
                oldAxis._bounds.top;
        location = ((oldAxis._bounds.top + oldAxis._bounds.height) -
                ((oldAxis._bounds.top - location).abs())) -
            textSize.height / 2;
      } else {
        location =
            ((_valueToCoefficient(value, oldAxis) * oldAxis._bounds.width) +
                    oldAxis._bounds.left)
                .roundToDouble();
        if (angle != null) {
  ...
  double _getShiftedPosition(ChartAxis axis, Rect axisBounds, double pointX,
      double pointY, Size textSize, int i) {
    if (axis.edgeLabelPlacement == EdgeLabelPlacement.shift) {
      if (i == 0 &&
          ((pointX < axisBounds.left && !axis.isInversed) ||
              (pointX + textSize.width > axisBounds.right &&
  ...
  void drawHorizontalAxesTitle(
    ...
    style = _getTextStyle(
        textStyle: style,
        fontColor: style.color ?? chart._chartState._chartTheme.axisTitleColor);
    ...
    if (axis.labelPosition == ChartDataLabelPosition.inside) {
      ...
      top = !axis.opposedPosition
          ? axisBounds.top +
              ...
              (!kIsWeb
                  ? axis._maximumLabelSize.height
                  : axis._maximumLabelSize.height + textSize.height)
          : axisBounds.top -
              (axis._isInsideTickPosition ? 0 : axis.majorTickLines.size) -
              innerPadding -
              axis._maximumLabelSize.height -
...
class _CartesianAxisRendererState extends State<_CartesianAxisRenderer>
  ...
  Widget build(BuildContext context) {
    widget.state = this;
    animationController.duration = const Duration(milliseconds: 0);
...
class _CartesianAxesPainter extends CustomPainter {
  ...
  void _onDraw(Canvas canvas) {
    if (renderType == 'outside') {
      ...
      if (chart.plotAreaBackgroundImage != null &&
          chart._chartState._backgroundImage != null) {
        paintImage(
            canvas: canvas,
  ...
  void _drawHorizontalAxes(Canvas canvas, ChartAxis axis,
      double animationFactor, ChartAxis oldAxis, bool needAnimate) {
    if (axis.isVisible) {
      ...
      if (axis.majorTickLines.width > 0 || axis.majorGridLines.width > 0) {
        axis._axisRenderer.drawHorizontalAxesTickLines(canvas, axis, chart,
            renderType, animationFactor, oldAxis, needAnimate);
      }
      axis._axisRenderer.drawHorizontalAxesLabels(canvas, axis, chart,
          renderType, animationFactor, oldAxis, needAnimate);
      if (renderType == 'outside') {
  ...
  void _drawVerticalAxes(Canvas canvas, ChartAxis axis, double animationFactor,
      ChartAxis oldAxis, bool needAnimate) {
    if (axis.isVisible) {
      if (axis.axisLine.width > 0 && renderType == 'outside') {
      ...
      if (axis._visibleLabels.isNotEmpty &&
        ...
        axis._axisRenderer.drawVerticalAxesTickLines(canvas, axis, chart,
            renderType, animationFactor, oldAxis, needAnimate);
      }
      axis._axisRenderer.drawVerticalAxesLabels(canvas, axis, chart, renderType,
          animationFactor, oldAxis, needAnimate);
      if (renderType == 'outside') {
  ...
  void _drawAxes(Canvas canvas) {
    ...
    for (int axisIndex = 0;
      ...
      axis._orientation == AxisOrientation.horizontal
          ? _drawHorizontalAxes(
              canvas, axis, animationFactor, oldAxis, needAnimate)
          : _drawVerticalAxes(
              canvas, axis, animationFactor, oldAxis, needAnimate);
    }
  }
