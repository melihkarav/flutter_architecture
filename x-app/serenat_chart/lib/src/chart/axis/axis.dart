...
abstract class ChartAxis {
  ...
  ///Position of the dataLabel alignment in series.
  ///
  /// DataLabel can be placed either far or near or center
  ///
  ///Defaults to `LabelAlignment.center`.
  ///
  ///_Note:_ This property changes the alignment position in different series.
  ///
  ...
  void calculateRange(ChartAxis _axis) {
    ...
    for (int i = 0; i < visibleSeries.length; i++) {
      ...
      if (series._visible &&
          series._minimumX != null &&
          series._maximumX != null &&
          series._minimumY != null &&
          series._maximumY != null) {
        paddingInterval = 0;
        ...
        if (((_xAxis is DateTimeAxis || _xAxis is NumericAxis) &&
          ...
          series._minDelta = series._minDelta ??
              _calculateMinPointsDelta(_xAxis, visibleSeries, _chart);
          paddingInterval = series._minDelta / 2;
        }
  ...
  void _triggerLabelRenderEvent(String labelText, num labelValue) {
    ...
    ChartTextStyle fontStyle;
    fontStyle = ChartTextStyle(
...
class AxisLabel {
  ...
  ///
  /// The [ChartTextStyle] is used to customize the chart title text style.
  ///
  ///```dart
  ///Widget build(BuildContext context) {
  ///    return Container(
