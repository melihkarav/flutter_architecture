...
abstract class XyDataSeries<T, D> extends CartesianSeries<T, D> {
  ...
  void storeSeriesProperties(SfCartesianChart chart, int index) {
    List<Trendline> trendlineCollection;
    _chart = chart;
    _isRectSeries =
    ...
    trendlineCollection = this.trendlines;
    // calculates the tooltip region for trenlines in this series
    if (trendlineCollection != null &&
        chart.tooltipBehavior != null &&
        chart.tooltipBehavior.enable) {
      for (int j = 0; j < trendlineCollection.length; j++) {
        if (trendlineCollection[j]._isNeedRender) {
          if (trendlineCollection[j]._pointsData != null) {
            for (int k = 0;
                k < trendlineCollection[j]._pointsData.length;
                k++) {
              final CartesianChartPoint<dynamic> trendlinePoint =
                  trendlineCollection[j]._pointsData[k];
              _calculateTooltipRegion(trendlinePoint, index, this, chart,
                  trendlineCollection[j], j);
            }
          }
        }
      }
  ...
  void calculateRegionData(
    ...
    if ((!isFastLine ||
      ...
      if (series._isRectSeries) {
        ...
        _calculatePathSeriesRegion(point, pointIndex, series, chart, rect,
            markerHeight, markerWidth, sideBySideInfo);
      }
  ...
  void _calculateAverageModeValue(
    ...
    final XyDataSeries<dynamic, dynamic> series = this;
    ...
    if (_seriesType.contains('range') ||
        _seriesType.contains('hilo') ||
        _seriesType.contains('candle')) {
      final _FinancialSeriesBase<dynamic, dynamic> financialSeries = series;
      if (financialSeries.showIndicationForSameValues) {
