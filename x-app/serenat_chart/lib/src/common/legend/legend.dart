...
class _ChartLegend {
  ...
  void _calculateLegends(SfCartesianChart chart, int index,
    ...
    series._seriesName = series._seriesName ?? 'series $index';
    if (series.isVisibleInLegend &&
      ...
      if (chart.onLegendItemRender != null) {
        ...
        legendEventArgs.text = series.legendItemText ??
            (isTrendlineadded ? trendline._name : series._seriesName);
    ...
    if (chart.legend.legendItemBuilder == null) {
      if (chart is SfCartesianChart) {
        for (int i = 0; i < chart._chartSeries.visibleSeries.length; i++) {
          ...
          _calculateLegends(chart, i, series);
          if (series is CartesianSeries<dynamic, dynamic>) {
            ...
            if (cseries != null && cseries.trendlines != null) {
              for (int j = 0; j < cseries.trendlines.length; j++) {
                ...
                if (trendline.isVisibleInLegend)
                  chart._chartLegend
                      ._calculateLegends(chart, i, cseries, trendline, j);
              }
            }
          }
        }
  ...
  void _calculateIndicatorLegends() {
    ...
    for (int i = 0; i < chart.indicators.length; i++) {
      ...
      if (indicator.isVisible && indicator.isVisibleInLegend) {
        ...
        final _LegendRenderContext _legendRenderContext = _LegendRenderContext(
            series: indicator,
            seriesIndex: chart._chartSeries.visibleSeries.length + i,
