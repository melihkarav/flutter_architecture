  ...
  void showByPixel(double x, double y) {
    ...
    if (chartTooltip != null &&
      ...
      if (!(chart is SfCartesianChart) ||
        ...
        chartTooltip?.state?._showTooltip(x, y);
...
// Defaults to true.
  ...
  void show(dynamic x, double y, [String xAxisName, String yAxisName]) {
    ...
    if (chart.tooltipBehavior._tooltipTemplate == null) {
      if (isInsidePointRegion ?? false) {
        chartTooltip?.state?._needMarker = true;
        chartTooltip?.state?._showTooltip(position.x, position.y);
      } else {
  ...
  void showByIndex(int seriesIndex, int pointIndex) {
    ...
    if (chart is SfCartesianChart) {
      ...
      if (chart.tooltipBehavior.builder != null &&
          ...
          pointIndex < chart.series[seriesIndex]._dataPoints.length &&
        ...
        pointIndex < chart.series._dataPoints.length) {
        ...
        if (series._visible &&
          ...
          series._regionalData.forEach((dynamic regionRect, dynamic values) {
            ...
            if (paddedRegion.contains(position)) {
              seriesIndex = i;
              pointIndex = series._dataPoints.indexOf(regionRect[4]);
              ...
              chart.tooltipBehavior._tooltipTemplate.template =
                  chart.tooltipBehavior.builder(series.dataSource[j],
                      regionRect[4], series, pointIndex, i);
          ...
          if (chart.tooltipBehavior._isHovering && isTooltipRegion) {
            chart.tooltipBehavior._tooltipTemplate.state.prevTooltipValue =
                chart
                    .tooltipBehavior._tooltipTemplate.state.currentTooltipValue;
            chart.tooltipBehavior._tooltipTemplate.state.currentTooltipValue =
                TooltipValue(seriesIndex, pointIndex);
          }
          chart.tooltipBehavior._tooltipTemplate.show = isTooltipRegion;
  ...
  void hide() {
    if (builder != null) {
      ...
      chartTooltip?.state?.show = false;
      _painter?.currentTooltipValue = _painter?.prevTooltipValue = null;
      chartTooltip?.state?.tooltipRepaintNotifier?.value++;
