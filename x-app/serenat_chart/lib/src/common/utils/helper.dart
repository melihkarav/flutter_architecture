...
void _cartesianLegendToggleState(
  ...
  if (currentItem.trendline == null ||
      chart.series[currentItem.seriesIndex]._visible) {
    if (legendToggles.isNotEmpty) {
      for (int i = 0; i < legendToggles.length; i++) {
        final _LegendRenderContext item = legendToggles[i];
        if (currentItem.trendline != null &&
    ...
    if (!needSelect) {
      if (!(!currentItem.series._visible &&
          !chart._chartState.isTrendlineToggled)) {
        needSelect = false;
        if (currentItem.trendlineIndex != null) {
          chart.series[currentItem.seriesIndex]._minimumX = 1 / 0;
          chart.series[currentItem.seriesIndex]._minimumY = 1 / 0;
          chart.series[currentItem.seriesIndex]._maximumX = -1 / 0;
          chart.series[currentItem.seriesIndex]._maximumY = -1 / 0;
        }
        legendToggles.add(currentItem);
      }
    }
